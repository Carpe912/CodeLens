...
1. **库里所有向量列都是 `vector(1536)`**（见 `migrations/002`）——这是硬约束，
   维度对不上会直接报错。
2. **当前模型原生是 1024 维**：`qwen3.7-text-embedding` 的默认输出是 **1024**，
   必须显式传 `EMBED_DIMENSIONS=1536`（请求里的 `dimensions` 参数）把它撑到 1536，
   才能与既有列兼容。
3. **统一性**：所有增强表（`string_constants`、`url_patterns` 等）都用 1536 维。

> ⚠️ **别把「模型默认维度」当成 1536。** 旧文档写的是「模型默认输出 1536」，
> 那是上一代模型（`text-embedding-v4`）的性质；换成 `qwen3.7-text-embedding` 后
> 默认是 1024，**不显式指定维度就会与库里的列不兼容**。
> 详见 `llm/embeddings.ts` 的模块注释。
...
2. 确保模型支持该维度（`qwen3.7-text-embedding` 支持可配置维度，原生 1024）
3. 修改所有表的 vector 列定义
4. 重新索引所有数据
...
- ✅ 统一使用 1536 维向量
- ✅ 与 `qwen3.7-text-embedding` + `EMBED_DIMENSIONS=1536` 的配置一致
- ✅ 提供更高的语义搜索精度
- ✅ 使用 HNSW 索引保证查询性能
- ⚠️ 迁移后需要重新索引所有仓库
