...
**CodeLens** 是一个 AI 驱动的代码搜索和问答系统，为整个代码仓库提供**可复核**的智能代码理解能力。

**核心功能：**
- 🔍 **多策略代码搜索**：向量、精确匹配、模糊搜索、依赖感知、调用图遍历
- 💬 **智能问答**：基于 LLM 的上下文感知代码问答
- 🐛 **根因分析**：自动分析 bug 的根本原因
- 🏗️ **架构理解**：解释系统设计和组件关系
- 📊 **调用图可视化**：展示函数调用关系
- 🔄 **增量索引**：按变更文件更新代码库索引（Web 端触发，经索引队列执行）
...
> 交互入口只有 Web 端一种。历史上有过一版 VS Code 扩展（`apps/vscode-extension`），
> 2026-09-19 因不再使用而移除；它承担的搜索 / 问答 / 调用图能力均已由 Web 端覆盖。
...
**位置：** `apps/api/src/retrieval/multi-strategy-search.ts`
...
- **[04-implementation-guide.md](./framework-comparison-04-implementation.md)** - 实施路线图与代码示例
