...
> 🗓️ **设计阶段文档**：文中出现的 `Anthropic` / `claude-sonnet-4-6` 是**当时的选型背景与对比对象**，
> 保留以反映决策过程，**不代表当前配置**。
> 当前技术栈以 `ecosystem.config.js` + `docs/deployment/SERVER_RUNBOOK.md` 为准：
> LLM = DeepSeek `deepseek-chat`、Embedding = `qwen3.7-text-embedding`、
...
5. **[框架替换的边界：哪些模块不该替换](./framework-comparison-05-boundaries.md)**
   - 7 条可复用的判断准则（谁负责失败 / 静默失效区 / 抽象粒度 / 观测面 / 反向适配 / 依赖成本 / 校验者≠被校验者）
   - 9 处刻意保持原生实现的模块及逐项理由（embeddings、rerank、async、检索融合、会话记忆、引用校验…）
   - 1 处编排形态取舍：为什么不用预置 Agent（`createReactAgent` 已装但刻意不用）
   - 对照案例：`llm/client.ts` 为什么**该**换（含"行数账要拆开算"与不成立的两项收益）
   - 最隐蔽的一类：`withStructuredOutput` 用在校验处会把「查伪」变成「自证」
   - 触发重新评估的条件（7 条）+ 可执行的验证命令（2026-09-23 决策记录）
   - 其中第 7 条专门回答「三条 Agent 链路要不要合并进图」：要，但**顺序不能反**
     （先开图 → 观察真实流量 → 再合并；理由见该节）
