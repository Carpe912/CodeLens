...
> 🗓️ **设计阶段文档**：文中的 `Anthropic API` 成本估算与代码示例是**当时的方案推演**，
> **不代表当前配置**。当前 LLM 为 DeepSeek `deepseek-chat`，
...
> ⚠️ **本阶段未被实施，且已重新评估为「不该做」。** 把检索能力包装成 LangChain Tools
> 的目的通常是交给模型自主挑选，而本仓的 5 路召回是**一次融合**里的 5 条通道，
> 拆成 5 把工具会绕过融合层。结论与反悔条件见
> [框架替换的边界 §4.10](./framework-comparison-05-boundaries.md)。
> 下文保留当时的实施方案，用于对照决策过程。
...
import { MultiStrategySearch } from "../../retrieval/multi-strategy-search";
