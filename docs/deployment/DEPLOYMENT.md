...
>
> 部署路径已统一为 **`/root/CodeLens`**（文中历史遗留的 `/opt/codelens` 已全部替换）。

---

### 📚 文档导航

**[← 返回首页](../../README.md)** · **[🎓 学习指南](../guides/LEARNING_GUIDE.md)** · **[🚀 升级说明](../agent-unimplemented-design.md)** · **[📐 项目架构](../guides/PROJECT_OVERVIEW.md)**
...
- ✅ Agent 核心代码（多轮推理引擎）
- ✅ 阿里百炼 DashScope 集成（`qwen3.7-text-embedding` 嵌入、`qwen3.7-text-rerank` 精排）
- ✅ 向量维度升级（1024 → 1536）
...
```bash
# 上传 API 构建产物
scp -r apps/api/dist root@47.116.6.132:/root/CodeLens/apps/api/

# 上传前端构建产物（注意使用 dist/* 避免嵌套）
ssh root@47.116.6.132 "rm -rf /root/CodeLens/apps/web/dist && mkdir -p /root/CodeLens/apps/web/dist"
scp -r apps/web/dist/* root@47.116.6.132:/root/CodeLens/apps/web/dist/

# 上传配置文件
scp package.json pnpm-workspace.yaml ecosystem.config.js root@47.116.6.132:/root/CodeLens/
```
  ...
  apps: [
    {
      ...
      env_production: {
        ...
        EMBED_DIMENSIONS: '1536',   // 必须 1536：与库中 vector(1536) 列一致（模型原生 1024）
...
> 🗑️ **本节描述的问题已不存在。** 它针对的是 `agent_*` 那批表
> （`agent_executions` / `agent_lessons` / `agent_reflections` / `tool_calls` / `conversation_memory`）。
> 这些表的建表脚本 `apps/api/src/db/migrations/add_agent_tables.sql` 已随死代码清理删除
> （审计发现除 `agent_conversations` 外全部零代码引用）。
> **当前迁移入口**是 `apps/api/migrations/001~004`，由 `scripts/deploy.js` 的台账执行器按序应用。
>
> 保留原始记录仅作历史参考：
...
~~如果需要重新创建表：~~
```bash
# 已失效：add_agent_tables.sql 已删除，agent_* 表也已不再使用
# ssh root@47.116.6.132
...
curl -X POST https://dashscope.aliyuncs.com/compatible-mode/v1/embeddings \
  ...
  -H "Content-Type: application/json" \
  -d '{"model":"qwen3.7-text-embedding","input":"test","dimensions":1536}'
...
ssh root@47.116.6.132 "grep -o 'src=\"[^\"]*\"' /root/CodeLens/apps/web/dist/index.html"
...
// ecosystem.config.js
{
  ...
  cwd: '/root/CodeLens/apps/web',  // 确保路径正确，不是 dist/dist
}
```
...
**症状**: 部署后发现 `/root/CodeLens/apps/web/dist/dist/` 嵌套目录
...
# 验证目录结构
ssh root@47.116.6.132 "ls -la /root/CodeLens/apps/web/dist/"
...
```bash
# 部署前备份
cp -r /root/CodeLens /root/CodeLens.backup.$(date +%Y%m%d_%H%M%S)
```
...
# 恢复备份
rm -rf /root/CodeLens
mv /root/CodeLens.backup.20240315_120000 /root/CodeLens
...
- [SERVER_RUNBOOK.md](./SERVER_RUNBOOK.md) - **当前唯一权威的部署 / 验证流程**
- [LEARNING_GUIDE.md](../guides/LEARNING_GUIDE.md) - 完整学习指南
- [agent-unimplemented-design.md](../agent-unimplemented-design.md) - Agent 未实现能力的设计归档
- [package.json](../../package.json) - 依赖和脚本配置
- [ecosystem.config.js](../../ecosystem.config.js) - PM2 配置（**运行时环境变量的真正来源**）

> 🗑️ 已删除的文件（不要再引用）：`DEPLOY_NOW.md`、`FINAL_STATUS.md`、
> `apps/api/src/db/migrations/add_agent_tables.sql`。
> 那批 `agent_*` 表**除 `agent_conversations` 外全部零代码引用**，
> 已随死代码清理一并移除；当前迁移脚本只有 `apps/api/migrations/001~004`。
