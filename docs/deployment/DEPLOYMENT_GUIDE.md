...
> ⚠️ **本文档已过时（历史存档），请勿按此操作。**
>
> 当前正确的部署与验证流程见 **[SERVER_RUNBOOK.md](./SERVER_RUNBOOK.md)**。
>
> 本文档中以下内容已与代码不符：
> - `migrate-to-dashscope.sh` —— 已删除
> - `src/scripts/migrate-agent.cjs` —— 已删除
> - `agent_executions` / `agent_lessons` / `agent_reflections` /
>   `conversation_memory` / `tool_calls` / `agent_performance_stats` —— 相关建表
>   文件（`add_agent_tables.sql`）已删除
> - 服务器地址写作 `your-server`，未指向实际部署目标
> - Step 6 建议直接 `psql` 改向量维度 —— 会绕过 `schema_migrations` 台账，
>   且该操作现已由迁移 `002` 幂等处理
> - 文中内嵌了真实 API Key（泄漏面），请勿复制传播
...
- **Rerank 模型**: `bge-reranker-v2-m3` → DashScope `qwen3.7-text-rerank`（更好的中文支持）
