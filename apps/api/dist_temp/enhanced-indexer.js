...
export class EnhancedIndexer {
     ...
     */
    async indexFile(repoId, fileId, filePath, content) {
     ...
     */
    async indexFiles(repoId, files, options = {}) {
     ...
     */
    async storeEntities(repoId, fileId, filePath, astResult) {
     ...
     */
    async generateEmbeddings(repoId, fileId, astResult) {
     ...
     */
    async storeEmbedding(repoId, fileId, entityType, entityId, embedding) {
     ...
     */
    async reindexRepository(repoId, repoPath, options = {}) {
      ...
      ORDER BY path
    `, [repoId]);
        const files = await Promise.all(filesResult.rows.map(async (row) => {
