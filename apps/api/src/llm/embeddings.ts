...
import { embeddingCache, cacheStatsTracker } from '../cache/index.js';
 ...
 * - EMBED_MODEL: 嵌入模型，默认 `qwen3.7-text-embedding`（阿里百炼 DashScope）
 * - EMBED_DIMENSIONS: 向量维度，默认 **1536**
 *
 * ⚠️ 维度为什么必须显式指定 1536：
 *   `qwen3.7-text-embedding` 原生维度是 **1024**，而库里所有向量列都是
 *   `vector(1536)`（见 migrations/002）。靠 `dimensions: 1536` 把它撑到 1536
 *   才能与既有数据兼容。**改这个值会让库里全部向量失效**（维度不同直接报错，
 *   换模型则是语义空间不同、静默失效——见 docs/deployment/SERVER_RUNBOOK.md 第 9 节）。
 *   **换嵌入模型后必须重刷向量**：`pnpm --filter @codelens/api reembed`。
 *
 ...
 * @returns 嵌入向量（长度 1536，与库中 vector(1536) 列一致）
 *
 * @example
...
export async function generateEmbedding(text: string): Promise<number[]> {
  ...
  // 从环境变量读取模型配置
  const model = process.env.EMBED_MODEL || 'qwen3.7-text-embedding';
  // 维度必须与库里的 vector(1536) 列一致，故默认 1536 而非「交给模型默认」
  // （qwen3.7-text-embedding 原生 1024，不指定会与既有列不兼容）
  const dimensions = process.env.EMBED_DIMENSIONS ? parseInt(process.env.EMBED_DIMENSIONS) : 1536;
