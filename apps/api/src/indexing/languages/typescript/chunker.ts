 ...
 * - TypeScript 枚举（TSEnumDeclaration）
 ...
 * ```
 */
export function parseTsFile(filePath: string, code: string): ChunkResult {
  // 初始化结果容器
  ...
  traverse(ast, {
     ...
     * - export class Bar {}
     * - export const baz = 1
     * - export interface IFoo {} / export type T = ... / export enum E {}
     *
     * ⚠️ 后三种（interface / type / enum）是 2026-09-19 补的：它们能被 A、B 两个 visitor
     * 抽成 chunk，却**从不进入 `exports` 列表** —— 于是「这个符号被谁导出」这条信息
     * 只对函数/类/变量成立。补上后 `exports` 才真的等于「本文件导出的全部符号」。
     */
     ...
     * 提取枚举的名称、位置和完整定义
     *
     * 示例：enum Status { ACTIVE = 'active', INACTIVE = 'inactive' }
     *
     * ⚠️ 这个 visitor 是 2026-09-19 补的。在此之前 `TSEnumDeclaration` **不在 visitor 表里**，
     * 而 `enum` 声明既不是 `VariableDeclaration` 也不是 `ClassDeclaration` ⇒ 纯枚举文件
     * 抽不出任何 chunk ⇒ `indexer` 判它「无可解析代码」直接跳过 ⇒ **整份文件不落 `files` 表**。
     *
     * 实测：repo 33（testwire-frontend）的 `src/enum/*.ts` 共 24 个文件 + `eumn.ts`（216 行）
     * 全部凭空消失 —— 而枚举恰恰是这个仓库里最该被搜到的东西（`HttpMethod`、
     * `TestCaseStepOperator`、`JsonSchemaType` 这类值域定义是读代码时的第一落点）。
     *
     * 注意：枚举**成员**另有去处（`entities.ts` 把每个成员收成 `string_constants`），
     * 这里只产一条「枚举声明」级别的块 —— 与 `interface` / `type` 的处理粒度一致。
     */
