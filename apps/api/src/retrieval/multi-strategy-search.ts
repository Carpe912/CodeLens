...
import { DependencyTracker } from '../indexing/dependency-tracker.js';
import { searchURL } from './url-search.js';
import { LRUCache, buildCacheKey } from '../cache/lru-cache.js';
import { generateSuggestions, correctTypos, SearchSuggestion } from './search-suggestions.js';
import { extractKeywords as extractMultilingualKeywords } from './multilingual-tokenizer.js';
import { deduplicateResults, DeduplicatableResult } from './deduplication.js';
import {
  rerankDocuments,
  isRerankEnabled,
  resolveRerankCandidates,
...
export class MultiStrategySearch {
   ...
   * 8. 去重过滤：移除重复结果
   * 9. 重排精排（可选）：`RERANK_ENABLED=true` 时先宽召回，再调 rerank 模型语义重排
   * 10. 生成建议：结果不足时提供搜索建议
   *
   * 说明：未开启 rerank 时，行为与历史版本完全一致（按规则分排序后截断）。
   *
   * @param repoId - 仓库 ID
   * @param query - 搜索查询
   * @param options - 搜索选项
  ...
  async searchWithSuggestions(
    ...
    // 检索宽度：开启 rerank 时先做「宽召回」，给精排留出候选池；否则维持原宽度。
    // 这点是关键——精排的意义在于「从一批排序不可靠的候选里挑最好的」，
    // 若仍按原 limit 召回，精排只能在一个已经被规则排序砍过的池子里重排，收益很小。
    const rerankOn = isRerankEnabled();
    const retrievalLimit = rerankOn ? Math.max(limit, resolveRerankCandidates()) : limit;
    ...
    // 步骤 2: 生成缓存键（使用纠正后的查询）
    const cacheKey = buildCacheKey('search', { repoId, query: searchQuery, ...options });
    ...
    if (intent.type === 'url') {
      ...
      try {
        const urlResults = await searchURL(this.db, repoId, query, retrievalLimit);
        ...
        // URL 分支同样走精排，保持与通用检索一致；并按 limit 截断
        // （宽召回时 searchURL 会返回更多候选，这里补回原来的截断语义）。
        const urlReranked = rerankOn
          ? await this.applyRerank(query, convertedResults)
          : convertedResults;
        const urlFinal = urlReranked.slice(0, limit);

        // 缓存并返回
        this.searchCache.set(cacheKey, urlFinal);
        return {
          results: urlFinal,
          correctedQuery: hasCorrected ? correctedQuery : undefined,
        };
    ...
    const searchResults = await Promise.all(
      selectedStrategies.map((strategy) => this.executeStrategy(repoId, query, intent, strategy, retrievalLimit))
    );
    ...
    if (followDependencies) {
      // 传入 limit：补入条目的上限要和证据预算挂钩，否则会把真实命中挤出预算
      await this.expandWithDependencies(repoId, filteredResults, limit);
    }

    // 步骤 12: 去重
    const deduplicatedResults = this.deduplicateSearchResults(filteredResults);

    // 步骤 12.5: rerank 精排（宽召回 → 模型语义重排 → 截断）
    // 只有开启才走；失败时 applyRerank 内部已降级回规则排序，不会让检索整体失败。
    const rerankedResults = rerankOn
      ? await this.applyRerank(searchQuery, deduplicatedResults)
      : deduplicatedResults;
  ...
  private async fuzzySearch(
    ...
    // 仓库范围必须走 files.repo_id：本项目的建表语句没有为 code_chunks 定义 repo_id。
    // （老库上可能残留一列同名 NULL 列，不能依赖它。）
  ...
  private buildVectorQuery(
    ...
    // ============================================================
    // 仓库范围一律走 files.repo_id，**不能**写成 t.repo_id
    // ============================================================
    // `code_chunks.repo_id` 是历史 schema 残留列，线上全为 NULL
    // （详见 db/index.ts:552-562 与 fuzzySearch 的同类注释）。
    // 一旦按 `t.repo_id = $1` 过滤，本表的向量召回会**静默返回 0 行**：
    // 不报错、不告警，只是 `code_chunks` 从候选池里整体消失。
    //
    // 后果极其隐蔽：`getSearchTables('general')` 明明把 code_chunks 列进来了，
    // 而「XX 功能是怎么实现的」这类问题**只有 code_chunks 存的是实现正文**
    // （functions 只有签名、string_constants 只有字面量）——
    // 于是问答看起来「有 10 条证据」，实则一条实现代码都没有，
    // 表现为「LLM 答案里提到了符号名，但证据里没有那段代码」。
    //
    // 这正是 db/index.ts 那句「新建库上会失败、老库上会静默返回 0 行」的实例。
    const fileIdColumn = table === 'url_patterns' ? 'definition_file_id' : 'file_id';
    const baseQuery = `
      SELECT
        t.*,
        f.path as file_path,
      ...
      FROM ${table} t
      JOIN files f ON t.${fileIdColumn} = f.id
      WHERE f.repo_id = $1
   ...
   * 组装送入 rerank 模型的候选文本
   *
   * 只保留「符号名 + 文件路径 + 代码内容」——rerank 模型看的是语义相关性，
   * 冗余的元数据既占 token 又干扰判断。长度上限由 rerank.ts 统一截断。
   */
  private buildRerankDocument(result: SearchResult): string {
    const symbol = result.context.symbolName || '';
    const header = symbol ? `${symbol} ${result.filePath}` : result.filePath;
    const maxDocChars = resolveRerankMaxDocChars();
    const body = result.content || '';
    // 先按上限留出 header 的空间，避免 header 被截断掉（符号名是强信号）
    const remaining = Math.max(0, maxDocChars - header.length - 1);
    return `${header}\n${body.slice(0, remaining)}`;
  }

  /**
   * rerank 精排：对规则排序后的候选调用 rerank 模型重新打分并重排
   *
   * 流程：
   * 1. 取前 RERANK_CANDIDATES 条候选（已按规则分排序，作为粗排结果）
   * 2. 调模型得到语义相关性分
   * 3. 用新分数覆盖 score，并把「重排前分数」存进 metadata 便于对比排查
   * 4. 未进入候选池的尾部结果原样接在后面（不丢数据）
   *
   * 失败降级：任何异常都只记录日志并原样返回规则排序，检索不会因此失败。
   */
  private async applyRerank(query: string, results: SearchResult[]): Promise<SearchResult[]> {
    if (results.length === 0) return results;

    const candidateCount = resolveRerankCandidates();
    const candidates = results.slice(0, candidateCount);
    const tail = results.slice(candidateCount);
    const documents = candidates.map((r) => this.buildRerankDocument(r));

    try {
      const scores = await rerankDocuments(query, documents, { topN: candidates.length });

      const reordered = scores
        .map((s) => ({ item: candidates[s.index], score: s.relevanceScore }))
        ...
        .map((entry) => ({
          ...
          metadata: {
            ...
            preRerankScore: entry.item.score,
          },
        }));

      console.log(
        `🔁 rerank 精排完成：候选 ${candidates.length} 条，top1 分数 ` +
          `${reordered[0]?.score.toFixed(4) ?? 'n/a'}（重排前 ${candidates[0]?.score.toFixed(4) ?? 'n/a'}）`
      );

      return [...reordered, ...tail];
    } catch (error) {
      console.warn(`⚠️ rerank 失败，回退规则排序：${(error as Error).message}`);
      return results;
    }
  }
  ...
  private async expandCodeContext(fileId: number, result: SearchResult): Promise<void> {
    ...
    // ⚠️ 必须在改写 lineStart/lineEnd **之前**记录，
    // 否则这里存下的是**扩展后**的行号，字段名 original* 就成了谎言。
    // （本字段当前无消费方，但错误的值比没有值更危险：将来谁拿它做精确定位，
    //   会静默拿到一个偏大的区间，正是本项目反复踩的「静默错误」类型。）
  ...
  /**
   * 依赖展开：把「命中代码所导入的符号」的**定义**补进结果集
   *
   * ============================================
   * 为什么需要它
   * ============================================
   * `followDependencies: true` 是 `/ask`、`/root-cause` 与 agent 图**默认打开**的开关
   * （`server/routes/ask.ts` 的 `ASK_RETRIEVAL_OPTIONS`、`agent/graph/nodes.ts`），
   * 而本方法此前是个**空函数** —— 函数体只有两句
   * `// This would add related files through dependency chains`。
   * 即：开关打开了、参数一路传到了、**什么都不会发生**。本项目最典型的「静默空转」。
   * 而 `docs/resume-codelens.md` 已经把「依赖展开」写进了多路召回的一路 —— 代码里没有。
   *
   * ============================================
   * 补的是什么（符号级，不是文件级）
   * ============================================
   * **不做**「把邻居文件的所有块都塞进来」：那样会瞬间淹没证据预算，而且大部分无关。
   * 只做一件事 —— 命中文件 F 从内部文件 T `import { X }` ⇒ 把 T 中**定义 X 的那个块**补进来。
   * 于是 LLM 看到的不只是「它调用了什么」，还有「被调用的东西是什么」。
   * 实测（repo 29，277 文件）：1336 条已解析目标文件的导入边里，**936 条（70%）**
   * 能按符号名对上一个真实的 `code_chunks` 行 —— 这就是本方法的召回升。
   *
   * ============================================
   * 三条硬约束（改动前请先读）
   * ============================================
   * 1. **不改原排名**：新条目插在它的「来源命中」**正后方**，其余命中的相对顺序不变。
   *    与 URL 关联层 `searchURLRelated()` 的既有约定一致
   *    （「排在直接命中之后、不改原排名、带标签、封顶」）。
   * 2. **必须有上限，且与 `limit` 挂钩**：`/ask` 只要 `limit: 10` 条证据。
   *    无上限地插入会把**真实命中挤出预算** —— 那样「覆盖」看着涨了、「精确」会掉。
   *    这里的上限是 `ceil(limit × 0.4)` 封顶 6。
   * 3. **绝不静默**：无论采纳几条都打一行带计数的日志；一条都没补上时要说清是
   *    「依赖图为空」还是「全部与已有证据重复」—— 否则「没生效」会伪装成「没有相关定义」。
   */
  private async expandWithDependencies(
    ...
    const MAX_FILES = 8;              // 最多扫描几个命中文件（控查询次数）
    const MAX_EDGES_PER_FILE = 12;    // 每个文件最多看几条导入边
    const SCORE_DISCOUNT = 0.6;       // 相关项不该压过直接命中
    // 与证据预算挂钩，见上文约束 2。limit=10 → 4 条；limit=15/20 → 6 条。
    const MAX_ADDITIONS = Math.max(2, Math.min(6, Math.ceil(limit * 0.4)));

    // 同一文件只取分数最高的那条当锚点，避免同一个文件被重复展开
    const anchorByFile = new Map<string, SearchResult>();
    for (const r of results) {
      const prev = anchorByFile.get(r.filePath);
      if (!prev || r.score > prev.score) anchorByFile.set(r.filePath, r);
    }
    const anchors = [...anchorByFile.values()].slice(0, MAX_FILES);

    const existing = new Set(results.map((r) => `${r.filePath}:${r.lineStart}`));
    const added: Array<{ anchorId: string; item: SearchResult }> = [];
    let edgesFound = 0;
    let skippedDup = 0;
    let hitCap = 0;

    for (const anchor of anchors) {
      if (added.length >= MAX_ADDITIONS) {
        hitCap++;
        continue;
      }

      const fileId = await this.getFileIdFromPath(repoId, anchor.filePath);
      if (!fileId) continue;

      // 只认「内部」导入边：imported_file_id 已解析 且 非外部依赖。
      // JOIN 到 code_chunks.symbol_name 做符号级定位 —— 这一步是本方法的价值所在。
      const defs = await this.db.query(
        `
        SELECT ir.imported_symbol   AS imported_symbol,
               ir.imported_file_id  AS imported_file_id,
               tf.path              AS imported_file_path,
               c.id                 AS chunk_id,
         ...
         ORDER BY ir.imported_symbol
         LIMIT $3
      `,
        [repoId, fileId, MAX_EDGES_PER_FILE]
      );

      edgesFound += defs.rows.length;
      ...
      for (const row of defs.rows) {
        ...
        const key = `${row.imported_file_path}:${row.line_start}`;
        if (existing.has(key)) {
          skippedDup++;
          continue;
        }
        ...
        const type: SearchResult['type'] =
          row.symbol_type === 'function' || row.symbol_type === 'method'
            ? 'function'
            : row.symbol_type === 'class'
              ? 'class'
              : 'chunk';
        ...
        const item: SearchResult = {
          id: `dep:${row.chunk_id}`,
          type,
          score: Math.max(0.01, anchor.score * SCORE_DISCOUNT),
          filePath: row.imported_file_path,
          lineStart: row.line_start,
          lineEnd: row.line_end,
          content: row.code_text,
          context: {
            fileName: row.imported_file_path.split('/').pop() || '',
            symbolName: row.symbol_name,
          },
          metadata: {
            // 打标签：让前端/证据映射能区分「直接命中」与「依赖补入」
            relation: 'dependency',
            definitionOf: row.imported_symbol,
            dependencyOfFile: anchor.filePath,
            sourceResultId: anchor.id,
          },
        };

        // 与其它证据保持一致：展开 ±5 行上下文（/ask 的命中判定也是按区间算的）
        await this.expandCodeContext(row.imported_file_id, item);

        existing.add(key);
        added.push({ anchorId: anchor.id, item });
      }
    }

    if (added.length === 0) {
      // 一条都没补上时**必须出声**，否则「依赖展开没生效」会伪装成「没有相关定义」。
      if (edgesFound === 0 && anchors.length > 0) {
        console.warn(
          `⚠️  依赖展开：扫描了 ${anchors.length} 个命中文件，但没有一条可用的内部导入定义` +
            `（依赖图可能是空的 → 先跑 pnpm --filter @codelens/api rebuild-graph）`
        );
      } else {
        console.log(
          `🔗 依赖展开：找到 ${edgesFound} 条定义，但全部与已有证据重复，未新增（跳过重复 ${skippedDup} 条）`
        );
      }
      return;
    }

    // 按锚点分组，再插到各自「来源命中」的正后方 —— 不改动其余命中的相对顺序
    const byAnchor = new Map<string, SearchResult[]>();
    ...
    for (const r of results) {
      rebuilt.push(r);
      const extra = byAnchor.get(r.id);
      if (extra) rebuilt.push(...extra);
    }

    // 自检：万一有条目没找到锚点，补到末尾而不是静默丢掉
    const present = new Set(rebuilt.map((r) => r.id));
    const leftovers = added.filter((a) => !present.has(a.item.id)).map((a) => a.item);
    if (leftovers.length > 0) {
      console.warn(`⚠️  依赖展开：${leftovers.length} 条新证据没找到锚点，已补到末尾`);
      rebuilt.push(...leftovers);
    }

    // 原地替换：调用方的 filteredResults 是同一个数组引用，后续去重/精排都能看到新条目
    results.splice(0, results.length, ...rebuilt);

    console.log(
      `🔗 依赖展开：扫描 ${anchors.length} 个命中文件 → 新增 ${added.length}/${MAX_ADDITIONS} 条定义证据` +
        (hitCap > 0 ? `（${hitCap} 处因达上限未展开）` : '') +
        (skippedDup > 0 ? `，跳过重复 ${skippedDup} 条` : '')
    );
  }
  ...
  private async getAvailableTerms(repoId: number): Promise<string[]> {
    ...
    try {
      // Get function names
      // 注意：functions / classes 两张表的列是 name（与 full_name），没有 symbol_name；
      // 只有 string_constants 才有 symbol_name。此前这里三处都写 symbol_name，
      // 导致整个 try 块在第一条查询就抛错 → 建议词永远为空（被 catch 静默吞掉）。
      const functions = await this.db.query(
        `
        SELECT DISTINCT name
        FROM functions
        WHERE repo_id = $1 AND name IS NOT NULL
        LIMIT 500
      `,
        [repoId]
      );
      terms.push(...functions.rows.map((r) => r.name));
      ...
      terms.push(...classes.rows.map((r) => r.name));
