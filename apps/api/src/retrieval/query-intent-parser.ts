 ...
 * URL 判定：路径段内允许的字符（不含 `/`）。
 *
 * 这是「一个查询算不算 URL」的**唯一字符集来源** —— `/search` 与 `/ask`
 * 都通过下面的 `looksLikeUrlQuery()` 使用它，不要各自再写一份正则。
 */
const PATH_CHAR = '[A-Za-z0-9_\\-.:{}$]';

/**
 * 「至少两段」的路径查询（不锚定首尾，允许出现在句子中间，如 `GET /api/users`）。
 *
 * ⚠️ 历史缺陷（2026-09-19 由第二语料 fastify/repo 30 暴露）：
 * 旧实现是 `/\/[a-z]+\/[a-z]+/i`，要求两个**连续字母**段。
 * 于是 `/post/123`、`/users/42` 这类「末段是 ID」的真实 REST 路径**判不出来**，
 * `/search` 会落到通用向量分支、返回一堆变量声明噪声，正确答案一次都不出现；
 * 而 `/api/post/123` 因为第二段是字母就正常 —— 同一个目标两种行为。
 *
 * 第一语料 repo 29 的 37 条 URL 用例**全部**以 `/api/...` 起头，第二段即配对成功，
 * 所以这个缺陷被 36/37 的成绩长期掩盖。段内字符**必须包含数字**。
 */
const URL_PATH_QUERY_RE = new RegExp(`/${PATH_CHAR}+/${PATH_CHAR}+`);

/**
 * 判断一个查询是否应当走「URL 专用检索通道」。
 *
 * @param query - 用户原始查询
 * @returns 完整 URL（http/https）或以 `/` 开头、**至少两段**的路径查询
 *
 * @example
 * looksLikeUrlQuery('/users/42')       // true  —— 旧实现在这里是 false（bug）
 * looksLikeUrlQuery('/api/users/42')   // true
 * looksLikeUrlQuery('GET /api/users')  // true  —— 允许出现在句子中间
 * looksLikeUrlQuery('getUserById')     // false
 */
