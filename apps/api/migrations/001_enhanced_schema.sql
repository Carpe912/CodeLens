...
CREATE INDEX IF NOT EXISTS idx_string_constants_repo ON string_constants(repo_id);
CREATE INDEX IF NOT EXISTS idx_string_constants_file ON string_constants(file_id);
CREATE INDEX IF NOT EXISTS idx_string_constants_symbol ON string_constants(symbol_name);
CREATE INDEX IF NOT EXISTS idx_string_constants_type ON string_constants(constant_type);
CREATE INDEX IF NOT EXISTS idx_string_constants_value ON string_constants USING gin(string_value gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_string_constants_embedding ON string_constants USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100);
...
CREATE INDEX IF NOT EXISTS idx_url_patterns_repo ON url_patterns(repo_id);
CREATE INDEX IF NOT EXISTS idx_url_patterns_pattern ON url_patterns USING gin(pattern gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_url_patterns_normalized ON url_patterns USING gin(normalized_pattern gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_url_patterns_method ON url_patterns(method);
CREATE INDEX IF NOT EXISTS idx_url_patterns_embedding ON url_patterns USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100);
...
CREATE TABLE IF NOT EXISTS functions (
  ...
  full_name TEXT,
  signature TEXT,
  -- TEXT 而非 VARCHAR(255)：这里存的是 ts-morph **推断**出来的返回类型，
  -- 组合式函数/大对象字面量动辄数百字符，超 255 会让 storeEntities 的整个事务回滚
  -- ⇒ 该文件在实体层（functions/classes/string_constants）整体消失。
  -- 详见 migrations/005_widen_function_return_type.sql。
  return_type TEXT,
...
CREATE INDEX IF NOT EXISTS idx_functions_full_name ON functions USING gin(full_name gin_trgm_ops);
...
CREATE INDEX IF NOT EXISTS idx_functions_embedding ON functions USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100);
...
CREATE INDEX IF NOT EXISTS idx_classes_embedding ON classes USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100);
...
CREATE INDEX IF NOT EXISTS idx_import_relations_repo ON import_relations(repo_id);
CREATE INDEX IF NOT EXISTS idx_import_relations_importer ON import_relations(importer_file_id);
CREATE INDEX IF NOT EXISTS idx_import_relations_imported ON import_relations(imported_file_id);
CREATE INDEX IF NOT EXISTS idx_import_relations_symbol ON import_relations(imported_symbol);
CREATE INDEX IF NOT EXISTS idx_import_relations_path ON import_relations USING gin(import_path gin_trgm_ops);
...
CREATE INDEX IF NOT EXISTS idx_constant_references_repo ON constant_references(repo_id);
CREATE INDEX IF NOT EXISTS idx_constant_references_constant ON constant_references(constant_id);
CREATE INDEX IF NOT EXISTS idx_constant_references_referrer ON constant_references(referrer_file_id);
...
CREATE INDEX IF NOT EXISTS idx_url_usages_repo ON url_usages(repo_id);
CREATE INDEX IF NOT EXISTS idx_url_usages_pattern ON url_usages(url_pattern_id);
CREATE INDEX IF NOT EXISTS idx_url_usages_file ON url_usages(usage_file_id);
...
CREATE INDEX IF NOT EXISTS idx_file_dependencies_repo ON file_dependencies(repo_id);
...
CREATE INDEX IF NOT EXISTS idx_search_logs_repo ON search_logs(repo_id);
CREATE INDEX IF NOT EXISTS idx_search_logs_query ON search_logs USING gin(query gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_search_logs_created ON search_logs(created_at);
CREATE INDEX IF NOT EXISTS idx_search_logs_type ON search_logs(query_type);
...
-- 注意：CREATE TRIGGER 没有 IF NOT EXISTS，重复执行会报 42P07。
-- 本文件必须可重放（台账为空时会被从头执行一次），因此先 DROP 再建。
DROP TRIGGER IF EXISTS trigger_update_file_dependency ON import_relations;
