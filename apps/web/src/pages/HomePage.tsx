...
export function HomePage() {
  ...
  // 与 reindex 的区别是**代价**：reindex 与仓库大小成正比（全量重建 + 重新生成向量），
  // resume 与剩余工作量成正比。所以「索引失败」时的第一反应应该是续跑，不是重建。
  const resumeRepoMutation = useMutation({
    mutationFn: async (repoId: number) => {
      const res = await fetch(`${API_BASE}/repos/${repoId}/resume`, {
        method: 'POST',
      });
      if (!res.ok) {
        const error = await res.json();
        throw new Error(error.error || 'Failed to resume indexing');
      }
      return res.json() as Promise<{ jobId: string; pendingFiles: number }>;
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['repos'] });
      setToast({
        message:
          data.pendingFiles > 0
            ? `正在续跑：待补 ${data.pendingFiles} 个文件（已完成的不会重做）`
            : '正在续跑：没有待补文件，仅重算依赖关系',
        type: 'success',
      });
    },
    onError: (error: Error) => {
      setToast({ message: `续跑失败: ${error.message}`, type: 'error' });
    },
  });
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    {/* 断点续跑：对 gitlab 与 zip 源都可用（zip 的工作区若已被清理会给出明确报错）。
                        与右边「重新索引」的区别是**不清空任何数据**，只补上次没做完的文件，
                        所以它在失败的仓库上应该比「重新索引」更先被点。 */}
                    {(repo.status === 'ready' || repo.status === 'failed') && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          resumeRepoMutation.mutate(repo.id);
                        }}
                        disabled={resumeRepoMutation.isPending}
                        className="p-2 rounded-lg bg-slate-800 hover:bg-amber-500/20 border border-slate-600 hover:border-amber-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed group/resume"
                        title="断点续跑（不清空数据，只补上次没做完的部分）"
                      >
                        <svg
                          className="w-4 h-4 text-gray-400 group-hover/resume:text-amber-400 transition-colors"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
                    ...
                    {repo.source === 'gitlab' && (repo.status === 'ready' || repo.status === 'failed') && (
                      <>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
