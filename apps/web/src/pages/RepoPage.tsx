    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      <div className="max-w-7xl mx-auto p-6">
        ...
        {loading && searchStatus && (
          <div className="mb-6 p-4 bg-blue-500/10 border border-blue-500/30 rounded-xl backdrop-blur-xl">
            <div className="flex items-center gap-3">
              <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-blue-400"></div>
              <span className="text-blue-300 font-medium">{searchStatus}</span>
            </div>
          </div>
        )}
        ...
        {result && (
          <div className="space-y-6">
            {result.answer && (
              <div className="bg-slate-800/40 backdrop-blur-xl rounded-xl p-6 border border-slate-700/50 shadow-xl">
                ...
                {!showFollowUpInput ? (
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-600/30 space-y-3">
                    <input
                      ...
                      disabled={loading}
                      className="w-full px-4 py-2 bg-slate-900/50 border border-slate-600 rounded-lg focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all disabled:opacity-50 text-gray-200 placeholder-gray-500 text-sm"
                    />
                    <div className="flex gap-2">
                      <button
                        onClick={() => handleSubmit(true)}
                        disabled={loading || !followUpQuery.trim()}
                        className="flex-1 bg-gradient-to-r from-cyan-500 to-blue-600 text-white py-2 px-4 rounded-lg hover:from-cyan-600 hover:to-blue-700 disabled:from-gray-600 disabled:to-gray-600 disabled:cursor-not-allowed font-medium transition-all text-sm"
                      >
                        {loading ? <LoadingSpinner /> : '提交'}
                      </button>
                      <button
                        ...
                        disabled={loading}
                        className="px-4 py-2 border border-slate-600 rounded-lg hover:bg-slate-700/50 disabled:cursor-not-allowed transition-all text-gray-300 text-sm"
                      >
                        取消
                      </button>
                    </div>
                    ...
                    <div className="space-y-2">
                      {result.historicalFeedback.map((item, idx) => (
                        <div key={idx} className="bg-slate-700/20 rounded-lg p-3 border border-slate-600/30">
                          <p className="text-xs text-gray-400 mb-2">相关问题: {item.query}</p>
                  ...
                  <div className="mt-6 pt-6 border-t border-slate-600/30">
                    {!showFeedbackForm ? (
                      <button
                        ...
                        className="flex items-center gap-2 text-cyan-400 hover:text-cyan-300 font-medium transition-colors text-sm"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <div className="space-y-3">
                        ...
                        <textarea
                          ...
                          className="w-full px-4 py-3 bg-slate-900/50 border border-slate-600 rounded-lg focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all disabled:opacity-50 text-gray-200 placeholder-gray-500 resize-none text-sm"
                          rows={3}
                        />
                        <div className="flex gap-2">
                          <button
                            ...
                            className="flex-1 bg-gradient-to-r from-green-500 to-emerald-600 text-white py-2 px-4 rounded-lg hover:from-green-600 hover:to-emerald-700 disabled:from-gray-600 disabled:to-gray-600 disabled:cursor-not-allowed font-medium transition-all text-sm"
                          >
                          ...
                          <button
                            ...
                            disabled={feedbackSubmitting || !feedbackText.trim()}
                            className="flex-1 bg-gradient-to-r from-orange-500 to-red-600 text-white py-2 px-4 rounded-lg hover:from-orange-600 hover:to-red-700 disabled:from-gray-600 disabled:to-gray-600 disabled:cursor-not-allowed font-medium transition-all text-sm"
                          >
          ...
          <div className="bg-slate-800/40 backdrop-blur-xl rounded-xl p-6 border border-slate-700/50 shadow-xl">
              ...
              <div className="flex gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    className="px-3 py-1.5 bg-slate-900/50 border border-slate-600 rounded-lg text-sm text-gray-300 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all"
                  >
            ...
            {filteredEvidence.length === 0 ? (
              <div className="text-gray-400 text-center py-8 bg-slate-900/20 rounded-lg border border-slate-700/30">
              ...
              <>
                <div className="space-y-3">
                  {paginatedEvidence.map((hit: SearchHit, i: number) => {
                    ...
                    return (
                      <div key={hit.id} className="bg-slate-900/30 border border-slate-700/40 rounded-lg overflow-hidden hover:border-cyan-500/40 transition-all">
                        <div
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3 flex-1 min-w-0">
                              <span className="inline-flex items-center justify-center w-6 h-6 rounded bg-gradient-to-br from-cyan-500 to-blue-600 text-white text-xs font-bold flex-shrink-0">
                                {startIndex + i + 1}
                              </span>
                              <div className="flex items-center gap-2 min-w-0 flex-1">
                                <span className="px-2 py-0.5 bg-blue-500/20 border border-blue-500/30 rounded text-blue-400 font-mono text-xs flex-shrink-0">
                                  {hit.symbol_type}
                                </span>
                                <span className="text-gray-300 text-sm font-medium truncate">{highlightText(hit.symbol_name, query)}</span>
                              </div>
                            </div>
                            <div className="flex items-center gap-2 flex-shrink-0 ml-3">
                              ...
                              <button
                                ...
                                className="px-3 py-1 bg-purple-500/20 border border-purple-500/30 text-purple-400 rounded hover:bg-purple-500/30 transition-all text-xs font-medium"
                              >
                                调用图
                              </button>
                              <svg
                          ...
                          <div className="mt-2 text-xs text-gray-400 flex items-center gap-1">
                            ...
                            <span className="text-gray-500 flex-shrink-0">:{hit.line_start}-{hit.line_end}</span>
                          </div>
                        </div>
                ...
                {totalPages > 1 && (
                  <div className="flex items-center justify-center gap-2 mt-6">
                    <button
                      ...
                      className="px-3 py-1.5 bg-slate-700/40 border border-slate-600 rounded-lg hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed text-gray-300 transition-all text-sm"
                    >
                      上一页
                    </button>
                            ...
                            <button
                              ...
                              className={`px-3 py-1.5 rounded-lg font-medium transition-all text-sm ${
                                currentPage === page
                                  ...
                                  : 'bg-slate-700/40 border border-slate-600 text-gray-300 hover:bg-slate-700'
                              }`}
                            >
                              {page}
                            </button>
